Feature: Print absolute paths
  ...
  Scenario: Add option in command line
    ...
    Then the output should contain:
      """
        Scenario:                                    # {{PATH:$CWD/features/test.feature}}:3
          ...
          And I have a step that throws an exception # FeatureContext::iHaveAFailingStep()
            Warning: Undefined variable $b in {{PATH:$CWD/features/bootstrap/FeatureContext.php}} line 16

      --- Failed scenarios:

          {{PATH:$CWD/features/test.feature}}:3
      """
